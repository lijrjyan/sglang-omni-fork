...
class OmniScheduler:
    ...
    def _enqueue_built_request(
        ...
        *,
        ...
        if bridge is not None and req_id in bridge.requests:
            ...
            if error:
                self._emit_request_error(req_id, ValueError(error))
                bridge.rollback(req_id)
                self.abort(req_id)
                return
        req = req_data.req
        ...
        def enqueue_if_live() -> None:
            ...
            if bridge is not None and req_id in bridge.requests:
                bridge.requests[req_id].unit.enqueued = True
    ...
    def abort(self, request_id: str, *, defer_running_cleanup: bool = True) -> None:
        ...
        with self._request_admission_lock:
            ...
            # Note (Junnan Li): A unit the bridge is cancelling keeps its rows and KV;
            # the unit completes normally and hands the KV back to its session.
            keep_session_rows = bridge is not None and bridge.cancelling == request_id
            immediate_reqs = (
                []
                if running_abort or keep_session_rows
    ...
    def _process_owned_async(self, pending) -> None:
    ...
    def _resolve_pending_async(self) -> None:
        ...
        if self._session_bridge is not None:
            # Note (Junnan Li): Wait before clearing a slot so a failed wait retains
            # ownership; clear before callbacks so recursive abort cannot collect
            # the same result twice.
            if self._async_previous is not None:
                self._wait_async_device(
                    self._async_previous[0], self._async_previous[2]
                )
                pending, self._async_previous = self._async_previous, None
                self._process_owned_async(pending)
            if self._async_pending is not None:
                self._wait_async_device(self._async_pending[0], self._async_pending[2])
                pending, self._async_pending = self._async_pending, None
                self._process_owned_async(pending)
            return
                ...
                try:
                    ...
                    if prev_pending is not None:
                        if self._session_bridge is not None:
                            ...
                            self._wait_async_device(prev_pending[0], prev_pending[2])
                            self._async_previous = None
                            self._process_owned_async(prev_pending)
                        else:
